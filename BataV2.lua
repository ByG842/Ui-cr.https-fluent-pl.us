...
ElementsTable.ProgressBar = (function()
	...
	function Element:New(Idx, Config)
		...
		function Bar:SetValue(Value)
			...
			TweenService:Create(FillBar, TweenInfo.new(0.35, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
		...
		function Bar:Destroy() BarFrame.Frame:Destroy() Library.Options[Idx] = nil end
...
ElementsTable.Badge = (function()
	...
	function Element:New(Idx, Config)
		assert(Config.Title, "Badge - Missing Title")
		Config.BadgeText = Config.BadgeText or "NEW"
		Config.BadgeColor = Config.BadgeColor or Color3.fromRGB(96, 205, 255)

		local Badge = { Type = "Badge", Value = Config.BadgeText }

		local BadgeFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		Badge.SetTitle = BadgeFrame.SetTitle
		Badge.SetDesc = BadgeFrame.SetDesc
		Badge.Visible = BadgeFrame.Visible
		Badge.Elements = BadgeFrame
		...
		local Pill = New("Frame", {
			...
			Parent = BadgeFrame.Frame,
		}, {
			New("UICorner", { CornerRadius = UDim.new(1, 0) }),
			New("UIPadding", { PaddingLeft = UDim.new(0, 8), PaddingRight = UDim.new(0, 8) }),
			New("UIStroke", { Color = Config.BadgeColor, Transparency = 0.4, ApplyStrokeMode = Enum.ApplyStrokeMode.Border }),
			New("TextLabel", {
		...
		function Badge:SetBadge(Text, Color)
			Pill:FindFirstChildOfClass("TextLabel").Text = Text
			self.Value = Text
			if Color then
				Pill.BackgroundColor3 = Color
				Pill.UIStroke.Color = Color
			end
		end
		function Badge:Destroy() BadgeFrame.Frame:Destroy() Library.Options[Idx] = nil end

		Library.Options[Idx] = Badge
		return Badge
	end
	return Element
end)()

-- [3] Separator (styled divider)
ElementsTable.Separator = (function()
	...
	function Element:New(Idx, Config)
		...
		function Sep:Destroy() Root:Destroy() Library.Options[Idx] = nil end
		Library.Options[Idx] = Sep
		return Sep
	end
	return Element
end)()
...
ElementsTable.Rating = (function()
	...
	function Element:New(Idx, Config)
		...
		Rating.SetDesc = RatingFrame.SetDesc
		Rating.Visible = RatingFrame.Visible
		Rating.Elements = RatingFrame
		...
		local function UpdateStars(val)
			for i, star in ipairs(Stars) do
				TweenService:Create(star, TweenInfo.new(0.15), {
					...
					Size = UDim2.fromOffset(i <= val and 20 or 16, i <= val and 20 or 16),
				}):Play()
			end
		end
		...
		for i = 1, Config.Max do
			local Star = New("ImageButton", {
				Image = "rbxassetid://10734966248", -- lucide-star
				Size = UDim2.fromOffset(16, 16),
				BackgroundTransparency = 1,
			...
			Creator.AddSignal(Star.MouseButton1Click, function()
				local newVal = i == Rating.Value and 0 or i
				Rating.Value = newVal
				UpdateStars(newVal)
				Library:SafeCallback(Rating.Callback, newVal)
				Library:SafeCallback(Rating.Changed, newVal)
			end)
		...
		function Rating:SetValue(val)
			val = math.clamp(val, 0, self.Max)
			self.Value = val
			UpdateStars(val)
		...
		function Rating:Destroy() RatingFrame.Frame:Destroy() Library.Options[Idx] = nil end

		UpdateStars(Config.Default)
...
ElementsTable.Alert = (function()
	...
	function Element:New(Idx, Config)
		...
		function Alert:Destroy() Root:Destroy() Library.Options[Idx] = nil end
		Library.Options[Idx] = Alert
		return Alert
	end
	return Element
end)()
...
ElementsTable.Checkbox = (function()
	...
	function Element:New(Idx, Config)
		...
		function Checkbox:Destroy() CBFrame.Frame:Destroy() Library.Options[Idx] = nil end
	...
	function Element:New(Idx, Config)
		...
		local function UpdateRadio(selected)
			for _, btn in pairs(Buttons) do
				local isSelected = btn.Value == selected
		...
		for _, opt in ipairs(Config.Options) do
			...
			Creator.AddSignal(Row.MouseButton1Click, function()
				Radio.Value = opt
				UpdateRadio(opt)
				Library:SafeCallback(Radio.Callback, opt)
				Library:SafeCallback(Radio.Changed, opt)
			end)
		end
		...
		function Radio:OnChanged(Func) Radio.Changed = Func Func(Radio.Value) end
		function Radio:Destroy() RGFrame.Frame:Destroy() Library.Options[Idx] = nil end

		UpdateRadio(Radio.Value)
...
ElementsTable.NumberInput = (function()
	...
	function Element:New(Idx, Config)
		...
		function NI:Destroy() NIFrame.Frame:Destroy() Library.Options[Idx] = nil end
...
ElementsTable.CopyButton = (function()
	...
	function Element:New(Idx, Config)
		...
		function CB:Destroy() CBFrame.Frame:Destroy() Library.Options[Idx] = nil end
...
ElementsTable.ConfirmButton = (function()
	...
	function Element:New(Idx, Config)
		...
		local CFB = { Type = "ConfirmButton" }
		local CFBFrame = Components.Element(Config.Title, Config.Description, self.Container, true, Config)
		CFB.SetTitle = CFBFrame.SetTitle
		CFB.SetDesc  = CFBFrame.SetDesc
		CFB.Visible  = CFBFrame.Visible
		CFB.Elements = CFBFrame

		local confirmed = false
		local resetTask

		local Btn = New("TextButton", {
			Text = Config.ButtonText or "Click to Confirm",
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
			TextSize = 12,
		...
		Creator.AddSignal(Btn.MouseButton1Click, function()
			if not confirmed then
				...
				resetTask = task.delay(2.5, function()
					confirmed = false
					if Btn and Btn.Parent then
						Btn.Text = Config.ButtonText or "Click to Confirm"
		...
		function CFB:Destroy() CFBFrame.Frame:Destroy() Library.Options[Idx] = nil end
		Library.Options[Idx] = CFB
		return CFB
	end
	return Element
end)()

-- [11] StatCard (shows a stat with icon + value)
ElementsTable.StatCard = (function()
	...
	function Element:New(Idx, Config)
		...
		local SC = { Type = "StatCard", Value = Config.Value }
		...
		local ValLabel = New("TextLabel", {
			Text = tostring(Config.Value) .. Config.Suffix,
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold),
			TextSize = 20,
			Position = UDim2.new(1, -12, 0.5, 0),
			AnchorPoint = Vector2.new(1, 0.5),
		...
		function SC:SetValue(val)
			self.Value = val
			ValLabel.Text = tostring(val) .. (Config.Suffix or "")
		end
		function SC:Destroy() SCFrame.Frame:Destroy() Library.Options[Idx] = nil end

		Library.Options[Idx] = SC
		return SC
	end
	return Element
end)()

-- [12] MultiSlider (dual-handle range slider)
ElementsTable.MultiSlider = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "MultiSlider"

	function Element:New(Idx, Config)
		assert(Config.Title, "MultiSlider - Missing Title")
		Config.Min = Config.Min or 0
		Config.Max = Config.Max or 100
		...
		local MSFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		MS.SetTitle = MSFrame.SetTitle
		MS.SetDesc  = MSFrame.SetDesc
		MS.Visible  = MSFrame.Visible
		MS.Elements = MSFrame
		...
		local TrackBg = New("Frame", {
			...
			AnchorPoint = Vector2.new(0, 1),
			BackgroundTransparency = 0.5,
			Parent = MSFrame.Frame,
			ThemeTag = { BackgroundColor3 = "SliderRail" },
		}, { New("UICorner", { CornerRadius = UDim.new(1, 0) }) })

		local FillRange = New("Frame", {
			Size = UDim2.new(0, 0, 1, 0),
			BackgroundTransparency = 0,
			Parent = TrackBg,
			ThemeTag = { BackgroundColor3 = "Accent" },
		...
		local dragging = { false, false }

		local function updateVisual()
			local pct1 = (MS.Value[1] - MS.Min) / (MS.Max - MS.Min)
			local pct2 = (MS.Value[2] - MS.Min) / (MS.Max - MS.Min)
			Handles[1].Position = UDim2.new(pct1, -7, 0.5, 0)
			Handles[2].Position = UDim2.new(pct2, -7, 0.5, 0)
			FillRange.Position = UDim2.new(pct1, 0, 0, 0)
			FillRange.Size     = UDim2.new(pct2 - pct1, 0, 1, 0)
			ValLabel.Text = tostring(MS.Value[1]) .. " – " .. tostring(MS.Value[2])
		end

		for i = 1, 2 do
			Creator.AddSignal(Handles[i].InputBegan, function(inp)
				if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
					dragging[i] = true
				end
			end)
			Creator.AddSignal(Handles[i].InputEnded, function(inp)
				if inp.UserInputType == Enum.UserInputType.MouseButton1 or inp.UserInputType == Enum.UserInputType.Touch then
					dragging[i] = false
				end
			end)
		...
		Creator.AddSignal(UserInputService.InputChanged, function(inp)
			for i = 1, 2 do
				if dragging[i] then
					...
					local scale = math.clamp((pos.X - TrackBg.AbsolutePosition.X) / TrackBg.AbsoluteSize.X, 0, 1)
					local raw = MS.Min + (MS.Max - MS.Min) * scale
					local val = Library:Round(raw, MS.Rounding)
					if i == 1 then val = math.min(val, MS.Value[2]) else val = math.max(val, MS.Value[1]) end
					MS.Value[i] = val
					updateVisual()
					Library:SafeCallback(MS.Callback, MS.Value)
					Library:SafeCallback(MS.Changed, MS.Value)
				end
			end
		end)

		function MS:SetValue(v)
			self.Value = { math.clamp(v[1], self.Min, self.Max), math.clamp(v[2], self.Min, self.Max) }
			updateVisual()
			Library:SafeCallback(self.Callback, self.Value)
			Library:SafeCallback(self.Changed, self.Value)
		end
		function MS:OnChanged(Func) MS.Changed = Func Func(MS.Value) end
		function MS:Destroy() MSFrame.Frame:Destroy() Library.Options[Idx] = nil end

		updateVisual()
		Library.Options[Idx] = MS
		return MS
	end
	return Element
end)()

-- [13] Accordion (collapsible content section)
ElementsTable.Accordion = (function()
	...
	function Element:New(Idx, Config)
		...
		local Acc = { Type = "Accordion", Opened = Config.Open, Elements = {} }
		...
		local function toggle(open)
			...
			if open then
				Root.Size = UDim2.new(1, 0, 0, 36 + getContentHeight())
			else
		...
		Creator.AddSignal(Header.MouseButton1Click, function()
			toggle(not Acc.Opened)
		end)

		Acc.Container = ContentFrame

		function Acc:AddToggle(idx, cfg) return Elements.AddToggle({ Container = ContentFrame, ScrollFrame = self.ScrollFrame, Library = Library }, idx, cfg) end
		function Acc:AddSlider(idx, cfg) return Elements.AddSlider({ Container = ContentFrame, ScrollFrame = self.ScrollFrame, Library = Library }, idx, cfg) end
		function Acc:AddButton(cfg) return Elements.AddButton({ Container = ContentFrame, ScrollFrame = self.ScrollFrame, Library = Library }, nil, cfg) end
		function Acc:AddParagraph(cfg) return Elements.AddParagraph({ Container = ContentFrame, ScrollFrame = self.ScrollFrame, Library = Library }, nil, cfg) end
		function Acc:Open() toggle(true) end
		function Acc:Close() toggle(false) end
		function Acc:Destroy() Root:Destroy() Library.Options[Idx] = nil end

		if Config.Open then toggle(true) end
...
-- [14] Timeline (vertical event list)
ElementsTable.Timeline = (function()
	...
	function Element:New(Idx, Config)
		...
		for i, ev in ipairs(Config.Events) do
			...
			local Row = New("Frame", {
				...
				New("Frame", {
					...
					BackgroundTransparency = isLast and 1 or 0.6,
					AutomaticSize = Enum.AutomaticSize.Y,
					ThemeTag = { BackgroundColor3 = "TitleBarLine" },
				}),
				New("Frame", {
					...
					BackgroundTransparency = 0,
					ThemeTag = { BackgroundColor3 = "Accent" },
				}, { New("UICorner", { CornerRadius = UDim.new(1, 0) }), New("UIAspectRatioConstraint", { AspectRatio = 1 }) }),
				New("Frame", {
					Size = UDim2.new(1, -20, 0, 0),
					AutomaticSize = Enum.AutomaticSize.Y,
					...
					New("UIListLayout", { SortOrder = Enum.SortOrder.LayoutOrder }),
					New("UIPadding", { PaddingBottom = UDim.new(0, isLast and 0 or 10) }),
					New("TextLabel", {
						Text = ev.Label or "",
						FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
						TextSize = 13,
		...
		function TL:Destroy() TLFrame.Frame:Destroy() Library.Options[Idx] = nil end
		Library.Options[Idx] = TL
		return TL
	end
	return Element
end)()

-- [15] QuickActions (row of small icon buttons)
ElementsTable.QuickActions = (function()
	...
	function Element:New(Idx, Config)
		...
		function QA:Destroy() QAFrame.Frame:Destroy() Library.Options[Idx] = nil end
...
ElementsTable.ScrollableList = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "ScrollableList"

	function Element:New(Idx, Config)
		assert(Config.Title, "ScrollableList - Missing Title")
		Config.Items = Config.Items or {}
		Config.Height = Config.Height or 120
		...
		local SLFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		SL.SetTitle = SLFrame.SetTitle
		SL.SetDesc  = SLFrame.SetDesc
		SL.Visible  = SLFrame.Visible
		SL.Elements = SLFrame
		...
		local ListContainer = New("ScrollingFrame", {
			...
			Parent = SLFrame.LabelHolder,
			LayoutOrder = 3,
			ScrollBarThickness = 2,
			ScrollBarImageTransparency = 0.5,
			CanvasSize = UDim2.fromScale(0, 0),
			ScrollingDirection = Enum.ScrollingDirection.Y,
			ThemeTag = { BackgroundColor3 = "Element", ScrollBarImageColor3 = "Accent" },
		}, {
			New("UICorner", { CornerRadius = UDim.new(0, 5) }),
		...
		local function selectItem(val)
			SL.Value = val
			for _, btn in pairs(ItemButtons) do
				local isSelected = btn.ItemValue == val
				TweenService:Create(btn.Frame, TweenInfo.new(0.15), {
					BackgroundTransparency = isSelected and 0.7 or 1
				}):Play()
			end
			Library:SafeCallback(SL.Callback, val)
			Library:SafeCallback(SL.Changed, val)
		end
		...
		local function buildList(items)
			...
			for i, item in ipairs(items) do
				...
				local bd = { Frame = ItemBtn, ItemValue = item }
				table.insert(ItemButtons, bd)
				Creator.AddSignal(ItemBtn.MouseButton1Click, function() selectItem(item) end)
			end
		...
		function SL:SetItems(items)
			self.Items = items
			buildList(items)
		end
		function SL:SetValue(val) selectItem(val) end
		function SL:OnChanged(Func) SL.Changed = Func Func(SL.Value) end
		function SL:Destroy() SLFrame.Frame:Destroy() Library.Options[Idx] = nil end

		Library.Options[Idx] = SL
		return SL
	end
	return Element
end)()

-- [17] HotkeyRecorder (record key combination)
ElementsTable.HotkeyRecorder = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "HotkeyRecorder"

	function Element:New(Idx, Config)
		assert(Config.Title, "HotkeyRecorder - Missing Title")
		...
		local HRFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		HR.SetTitle = HRFrame.SetTitle
		HR.SetDesc  = HRFrame.SetDesc
		HR.Visible  = HRFrame.Visible
		HR.Elements = HRFrame
		...
		local DisplayLabel = New("TextLabel", {
			...
			TextXAlignment = Enum.TextXAlignment.Right,
			Parent = HRFrame.Frame,
			ThemeTag = { TextColor3 = "SubText" },
		})

		local RecBtn = New("TextButton", {
			Text = "Record",
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
			TextSize = 11,
		...
		-- Arrange display + button
		DisplayLabel.Position = UDim2.new(1, -72, 0.5, 0)
		DisplayLabel.Size = UDim2.fromOffset(80, 14)
		...
		Creator.AddSignal(RecBtn.MouseButton1Click, function()
			if HR.Recording then
				HR.Recording = false
				RecBtn.Text = "Record"
				TweenService:Create(RecBtn, TweenInfo.new(0.2), { BackgroundTransparency = 0.85 }):Play()
				if recConn then recConn:Disconnect() recConn = nil end
				DisplayLabel.Text = formatKeys(HR.Value)
				Library:SafeCallback(HR.Callback, HR.Value)
				Library:SafeCallback(HR.Changed, HR.Value)
			else
				HR.Recording = true
				HR.Value = {}
				heldKeys = {}
				RecBtn.Text = "● Stop"
				TweenService:Create(RecBtn, TweenInfo.new(0.2), { BackgroundTransparency = 0.6 }):Play()
				DisplayLabel.Text = "Press keys..."

				recConn = UserInputService.InputBegan:Connect(function(inp)
					if HR.Recording and inp.KeyCode ~= Enum.KeyCode.Unknown then
						if not table.find(heldKeys, inp.KeyCode) then
							table.insert(heldKeys, inp.KeyCode)
							HR.Value = table.clone(heldKeys)
							DisplayLabel.Text = formatKeys(HR.Value)
						end
					end
				end)
		...
		function HR:SetValue(v)
			self.Value = v
			DisplayLabel.Text = formatKeys(v)
		end
		function HR:OnChanged(Func) HR.Changed = Func Func(HR.Value) end
		function HR:Destroy() HRFrame.Frame:Destroy() Library.Options[Idx] = nil end

		Library.Options[Idx] = HR
		return HR
	end
	return Element
end)()
...
ElementsTable.Timer = (function()
	...
	function Element:New(Idx, Config)
		...
		function Tim:Destroy() stopTick() TFrame.Frame:Destroy() Library.Options[Idx] = nil end
...
ElementsTable.ImagePicker = (function()
	local Element = {}
	Element.__index = Element
	...
	function Element:New(Idx, Config)
		...
		Config.Columns = Config.Columns or 4

		local IP = {
			Type = "ImagePicker",
			Value = Config.Default or nil,
			Callback = Config.Callback or function() end,
		}

		local IPFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		IP.SetTitle = IPFrame.SetTitle
		IP.SetDesc  = IPFrame.SetDesc
		IP.Visible  = IPFrame.Visible
		IP.Elements = IPFrame

		local cellSize = 34
		local cols = Config.Columns
		local rows = math.ceil(#Config.Images / cols)
		local gridH = rows * cellSize + (rows - 1) * 4

		local Grid = New("Frame", {
			Size = UDim2.new(1, -20, 0, gridH),
			Position = UDim2.fromOffset(10, 0),
			BackgroundTransparency = 1,
			Parent = IPFrame.LabelHolder,
			LayoutOrder = 3,
		}, {
			New("UIGridLayout", {
				CellSize = UDim2.fromOffset(cellSize, cellSize),
				CellPaddingSize = UDim2.fromOffset(4, 4),
				SortOrder = Enum.SortOrder.LayoutOrder,
			}),
		...
		for i, img in ipairs(Config.Images) do
			local Btn = New("ImageButton", {
				Image = img.Image or img,
				Size = UDim2.fromOffset(cellSize, cellSize),
				BackgroundTransparency = 0.88,
				Parent = Grid,
				LayoutOrder = i,
				ThemeTag = { BackgroundColor3 = "Element", ImageColor3 = "Text" },
			}, {
				New("UICorner", { CornerRadius = UDim.new(0, 6) }),
			...
			Creator.AddSignal(Btn.MouseButton1Click, function()
				if selectedBtn then
					selectedBtn.UIStroke.Transparency = 1
					TweenService:Create(selectedBtn, TweenInfo.new(0.15), { BackgroundTransparency = 0.88 }):Play()
				end
				selectedBtn = Btn
				Btn.UIStroke.Transparency = 0
				TweenService:Create(Btn, TweenInfo.new(0.15), { BackgroundTransparency = 0.6 }):Play()
				IP.Value = img.Image or img
				Library:SafeCallback(IP.Callback, IP.Value)
				Library:SafeCallback(IP.Changed, IP.Value)
			end)
		end

		function IP:OnChanged(Func) IP.Changed = Func Func(IP.Value) end
		function IP:Destroy() IPFrame.Frame:Destroy() Library.Options[Idx] = nil end

		Library.Options[Idx] = IP
		return IP
	end
	return Element
end)()
...
ElementsTable.Stepper = (function()
	...
	function Element:New(Idx, Config)
		...
		for i, step in ipairs(Config.Steps) do
			if i > 1 then
				New("Frame", {
					Size = UDim2.new(1 / (#Config.Steps * 2), 0, 0, 2),
					BackgroundTransparency = 0.6,
					Parent = Holder,
					LayoutOrder = i * 10 - 1,
					ThemeTag = { BackgroundColor3 = "TitleBarLine" },
			...
			local col = New("Frame", {
				...
				New("UIListLayout", { HorizontalAlignment = Enum.HorizontalAlignment.Center, SortOrder = Enum.SortOrder.LayoutOrder }),
			})
			...
			local dot = New("Frame", {
				...
				New("UICorner", { CornerRadius = UDim.new(1, 0) }),
				New("UIAspectRatioConstraint", { AspectRatio = 1 }),
				New("UIStroke", { Transparency = 0.4, ApplyStrokeMode = Enum.ApplyStrokeMode.Border, ThemeTag = { Color = "Accent" } }),
				New("TextLabel", {
					Text = tostring(i),
					FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold),
					TextSize = 11,
		...
		local function updateDots()
			for i, dot in ipairs(Dots) do
				local active = i == ST.Value
				local done   = i < ST.Value
				TweenService:Create(dot, TweenInfo.new(0.2), {
					BackgroundTransparency = active and 0 or (done and 0.3 or 0.8)
				}):Play()
			end
			PrevBtn.Visible = ST.Value > 1
			NextBtn.Text = ST.Value == #Config.Steps and "✓ Done" or "Next →"
		end

		Creator.AddSignal(PrevBtn.MouseButton1Click, function()
			if ST.Value > 1 then
				ST.Value = ST.Value - 1
				updateDots()
				Library:SafeCallback(ST.Callback, ST.Value)
				Library:SafeCallback(ST.Changed, ST.Value)
			end
		end)
		Creator.AddSignal(NextBtn.MouseButton1Click, function()
			if ST.Value < #Config.Steps then
				ST.Value = ST.Value + 1
				updateDots()
				Library:SafeCallback(ST.Callback, ST.Value)
				Library:SafeCallback(ST.Changed, ST.Value)
			else
				Library:SafeCallback(Config.OnComplete)
			end
		end)

		function ST:SetStep(s)
			self.Value = math.clamp(s, 1, #self.Steps)
			updateDots()
			Library:SafeCallback(self.Callback, self.Value)
			Library:SafeCallback(self.Changed, self.Value)
		end
		function ST:OnChanged(Func) ST.Changed = Func Func(ST.Value) end
		function ST:Destroy() STFrame.Frame:Destroy() Library.Options[Idx] = nil end

		updateDots()
		Library.Options[Idx] = ST
		return ST
	end
	return Element
end)()

-- [21] TagInput (input with multi-tag chips)
ElementsTable.TagInput = (function()
	...
	function Element:New(Idx, Config)
		...
		Config.MaxTags = Config.MaxTags or 8
		Config.Placeholder = Config.Placeholder or "Type + Enter..."
		...
		local function removeTag(tag)
			for i, t in ipairs(TI.Value) do
				if t == tag then table.remove(TI.Value, i) break end
			end
			if chips[tag] then chips[tag]:Destroy() chips[tag] = nil end
		...
		local function addTag(tag)
			tag = tag:match("^%s*(.-)%s*$")
			if tag == "" or table.find(TI.Value, tag) or #TI.Value >= Config.MaxTags then return end
			table.insert(TI.Value, tag)
			local chip = New("TextButton", {
				...
				New("UICorner", { CornerRadius = UDim.new(1, 0) }),
				New("UIPadding", { PaddingLeft = UDim.new(0, 8), PaddingRight = UDim.new(0, 26) }),
				New("TextLabel", {
					Text = tag,
					FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
					TextSize = 11,
			...
			chip:FindFirstChild("Remove").MouseButton1Click:Connect(function() removeTag(tag) end)
			chips[tag] = chip
			Library:SafeCallback(TI.Callback, TI.Value)
			Library:SafeCallback(TI.Changed, TI.Value)
		end
		...
		}, { New("UICorner", { CornerRadius = UDim.new(1, 0) }), New("UIPadding", { PaddingLeft = UDim.new(0, 8), PaddingRight = UDim.new(0, 8) }) })
		...
		for _, t in ipairs(TI.Value) do addTag(t) end

		function TI:SetValue(tags)
			for tag in pairs(chips) do removeTag(tag) end
			for _, t in ipairs(tags) do addTag(t) end
		end
		function TI:OnChanged(Func) TI.Changed = Func Func(TI.Value) end
		function TI:Destroy() TIFrame.Frame:Destroy() Library.Options[Idx] = nil end

		Library.Options[Idx] = TI
		return TI
	end
	return Element
end)()

-- [22] ColorSwatch (shows a row of preset colors to pick)
ElementsTable.ColorSwatch = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "ColorSwatch"

	function Element:New(Idx, Config)
		assert(Config.Title, "ColorSwatch - Missing Title")
		Config.Colors = Config.Colors or {
			Color3.fromRGB(255,80,80), Color3.fromRGB(255,150,50),
			Color3.fromRGB(255,220,50), Color3.fromRGB(80,200,80),
			Color3.fromRGB(60,160,255), Color3.fromRGB(160,80,255),
			Color3.fromRGB(255,80,180), Color3.fromRGB(200,200,200),
		}

		local CS = {
			Type = "ColorSwatch",
			Value = Config.Default or Config.Colors[1],
			Callback = Config.Callback or function() end,
		}

		local CSFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		CS.SetTitle = CSFrame.SetTitle
		CS.SetDesc  = CSFrame.SetDesc
		CS.Visible  = CSFrame.Visible
		CS.Elements = CSFrame
		...
		local SwatchRow = New("Frame", {
			...
			AnchorPoint = Vector2.new(1, 0.5),
			BackgroundTransparency = 1,
			Parent = CSFrame.Frame,
		}, {
			New("UIListLayout", { FillDirection = Enum.FillDirection.Horizontal, Padding = UDim.new(0, 3), VerticalAlignment = Enum.VerticalAlignment.Center }),
		})
		...
		for _, col in ipairs(Config.Colors) do
			local Swatch = New("TextButton", {
				...
				New("UICorner", { CornerRadius = UDim.new(1, 0) }),
				New("UIAspectRatioConstraint", { AspectRatio = 1 }),
				New("UIStroke", { Transparency = 1, Color = Color3.fromRGB(255,255,255), ApplyStrokeMode = Enum.ApplyStrokeMode.Border }),
			})

			Creator.AddSignal(Swatch.MouseButton1Click, function()
				if selected then selected.UIStroke.Transparency = 1 end
				selected = Swatch
				Swatch.UIStroke.Transparency = 0
				CS.Value = col
				Library:SafeCallback(CS.Callback, col)
				Library:SafeCallback(CS.Changed, col)
			end)
		end

		function CS:SetValue(col) CS.Value = col Library:SafeCallback(self.Callback, col) end
		function CS:OnChanged(Func) CS.Changed = Func Func(CS.Value) end
		function CS:Destroy() CSFrame.Frame:Destroy() Library.Options[Idx] = nil end

		Library.Options[Idx] = CS
		return CS
	end
	return Element
end)()

-- [23] GradientBar (display a color gradient preview)
ElementsTable.GradientBar = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "GradientBar"

	function Element:New(Idx, Config)
		assert(Config.Title, "GradientBar - Missing Title")
		Config.Colors = Config.Colors or { Color3.fromRGB(96,205,255), Color3.fromRGB(255,100,200) }
		Config.Rotation = Config.Rotation or 90

		local GB = { Type = "GradientBar" }
		local GBFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		GB.SetTitle = GBFrame.SetTitle
		GB.SetDesc  = GBFrame.SetDesc
		GB.Visible  = GBFrame.Visible
		GB.Elements = GBFrame

		local kps = {}
		for i, col in ipairs(Config.Colors) do
			table.insert(kps, ColorSequenceKeypoint.new((i-1)/(#Config.Colors-1), col))
		end
		...
		function GB:Destroy() GBFrame.Frame:Destroy() Library.Options[Idx] = nil end
		Library.Options[Idx] = GB
		return GB
	end
	return Element
end)()

-- [24] InfoCard (card with icon + 2 lines)
ElementsTable.InfoCard = (function()
	...
	function Element:New(Idx, Config)
		...
		local IC = { Type = "InfoCard" }
		local ICFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		IC.SetTitle = ICFrame.SetTitle
		IC.SetDesc  = ICFrame.SetDesc
		IC.Visible  = ICFrame.Visible
		IC.Elements = ICFrame

		if Config.Value then
			local BigVal = New("TextLabel", {
				...
				Size = UDim2.fromOffset(0, 26),
				AutomaticSize = Enum.AutomaticSize.X,
				Parent = ICFrame.Frame,
				ThemeTag = { TextColor3 = "Accent" },
			})
			IC.ValueLabel = BigVal
		end

		function IC:SetValue(v)
			if self.ValueLabel then self.ValueLabel.Text = tostring(v) end
		end
		function IC:Destroy() ICFrame.Frame:Destroy() Library.Options[Idx] = nil end
		Library.Options[Idx] = IC
		return IC
	end
	return Element
end)()

-- [25] ButtonGroup (multiple buttons in one row)
ElementsTable.ButtonGroup = (function()
	...
	function Element:New(Idx, Config)
		...
		function BG:Destroy() BGFrame.Frame:Destroy() Library.Options[Idx] = nil end
...
ElementsTable.AnimatedLabel = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "AnimatedLabel"

	function Element:New(Idx, Config)
		assert(Config.Title, "AnimatedLabel - Missing Title")
		Config.Value = Config.Value or ""
		Config.Typewriter = Config.Typewriter or false

		local AL = { Type = "AnimatedLabel", Value = Config.Value }
		local ALFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		AL.SetTitle = ALFrame.SetTitle
		AL.SetDesc  = ALFrame.SetDesc
		AL.Visible  = ALFrame.Visible
		AL.Elements = ALFrame

		local ValLabel = New("TextLabel", {
			Text = Config.Typewriter and "" or Config.Value,
			FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Medium),
			TextSize = 13,
			Position = UDim2.new(1, -10, 0.5, 0),
			AnchorPoint = Vector2.new(1, 0.5),
		...
		if Config.Typewriter then
			task.spawn(function()
				for i = 1, #Config.Value do
					ValLabel.Text = Config.Value:sub(1, i)
					task.wait(0.04)
				end
			end)
		end

		function AL:SetValue(v, typewriter)
			self.Value = v
			if typewriter then
				ValLabel.Text = ""
				task.spawn(function()
					for i = 1, #v do
						ValLabel.Text = v:sub(1, i)
						task.wait(0.03)
					end
				end)
			else
				TweenService:Create(ValLabel, TweenInfo.new(0.2), { TextTransparency = 1 }):Play()
				task.delay(0.2, function()
					if ValLabel and ValLabel.Parent then
						ValLabel.Text = v
						TweenService:Create(ValLabel, TweenInfo.new(0.2), { TextTransparency = 0 }):Play()
					end
				end)
			end
		end
		function AL:Destroy() ALFrame.Frame:Destroy() Library.Options[Idx] = nil end
...
-- [27] Divider (simple visible line divider with optional label, different from Separator)
ElementsTable.Divider = (function()
	...
	function Element:New(Idx, Config)
		...
		function DV:Destroy() Root:Destroy() Library.Options[Idx] = nil end
		Library.Options[Idx] = DV
		return DV
	end
	return Element
end)()
...
ElementsTable.FavoriteButton = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "FavoriteButton"

	function Element:New(Idx, Config)
		assert(Config.Title, "FavoriteButton - Missing Title")
		...
		local FBFrame = Components.Element(Config.Title, Config.Description, self.Container, true, Config)
		FB.SetTitle = FBFrame.SetTitle
		FB.SetDesc  = FBFrame.SetDesc
		FB.Visible  = FBFrame.Visible
		FB.Elements = FBFrame

		local StarBtn = New("ImageButton", {
			Image = "rbxassetid://10734966248",
			...
			BackgroundTransparency = 1,
			ImageColor3 = Creator.GetThemeProperty("SubText"),
			Parent = FBFrame.Frame,
		})

		local function updateStar(val)
			TweenService:Create(StarBtn, TweenInfo.new(0.2), {
				ImageColor3 = val and Creator.GetThemeProperty("Accent") or Creator.GetThemeProperty("SubText"),
				Size = val and UDim2.fromOffset(24, 24) or UDim2.fromOffset(22, 22),
			}):Play()
		end

		Creator.AddSignal(FBFrame.Frame.MouseButton1Click, function()
			FB.Value = not FB.Value
			updateStar(FB.Value)
			Library:SafeCallback(FB.Callback, FB.Value)
			Library:SafeCallback(FB.Changed, FB.Value)
		end)

		function FB:SetValue(val)
			self.Value = val
			updateStar(val)
			Library:SafeCallback(self.Callback, val)
			Library:SafeCallback(self.Changed, val)
		end
		function FB:OnChanged(Func) FB.Changed = Func Func(FB.Value) end
		function FB:Destroy() FBFrame.Frame:Destroy() Library.Options[Idx] = nil end

		updateStar(Config.Default or false)
		Library.Options[Idx] = FB
		return FB
	end
	return Element
end)()
...
ElementsTable.TextCounter = (function()
	local Element = {}
	Element.__index = Element
	Element.__type = "TextCounter"

	function Element:New(Idx, Config)
		assert(Config.Title, "TextCounter - Missing Title")
		Config.MaxLength = Config.MaxLength or 100
		Config.Mode = Config.Mode or "chars" -- "chars" or "words"
		...
		local TCFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		TC.SetTitle = TCFrame.SetTitle
		TC.SetDesc  = TCFrame.SetDesc
		TC.Visible  = TCFrame.Visible
		TC.Elements = TCFrame
		...
		local CountLabel = New("TextLabel", {
			...
			TextXAlignment = Enum.TextXAlignment.Right,
			Parent = TCFrame.Frame,
			ThemeTag = { TextColor3 = "SubText" },
		})

		local TextArea = New("TextBox", {
		...
		local function getCount(txt)
			if Config.Mode == "words" then
				local words = 0
		...
		Creator.AddSignal(TextArea:GetPropertyChangedSignal("Text"), function()
			local txt = TextArea.Text
			if #txt > Config.MaxLength and Config.Mode == "chars" then
				txt = txt:sub(1, Config.MaxLength)
				TextArea.Text = txt
			end
			local count = getCount(txt)
			CountLabel.Text = count .. "/" .. Config.MaxLength
			CountLabel.TextColor3 = count >= Config.MaxLength and Color3.fromRGB(255, 80, 80) or Creator.GetThemeProperty("SubText")
			TC.Value = txt
			Library:SafeCallback(TC.Callback, txt)
			Library:SafeCallback(TC.Changed, txt)
		end)

		function TC:SetValue(v)
			self.Value = v
			TextArea.Text = v
		end
		function TC:OnChanged(Func) TC.Changed = Func Func(TC.Value) end
		function TC:Destroy() TCFrame.Frame:Destroy() Library.Options[Idx] = nil end

		Library.Options[Idx] = TC
		return TC
	end
	return Element
end)()
...
ElementsTable.Chip = (function()
	...
	function Element:New(Idx, Config)
		...
		function CH:Destroy() CHFrame.Frame:Destroy() Library.Options[Idx] = nil end
...
local SaveManager = {} do
	...
	SaveManager.Parser = {
		...
		MultiSlider = {
			Save = function(idx, object)
				return { type = "MultiSlider", idx = idx, v1 = object.Value[1], v2 = object.Value[2] }
			end,
			Load = function(idx, data)
				if SaveManager.Options[idx] then
					SaveManager.Options[idx]:SetValue({ data.v1, data.v2 })
				end
			end,
		},
		...
		TextCounter = {
			Save = function(idx, object)
				return { type = "TextCounter", idx = idx, text = object.Value }
			end,
			Load = function(idx, data)
				if SaveManager.Options[idx] then
					SaveManager.Options[idx]:SetValue(data.text)
		...
		FavoriteButton = {
			Save = function(idx, object)
				return { type = "FavoriteButton", idx = idx, value = object.Value }
			end,
			Load = function(idx, data)
				if SaveManager.Options[idx] then
					SaveManager.Options[idx]:SetValue(data.value)
