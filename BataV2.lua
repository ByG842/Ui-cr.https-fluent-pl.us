...
ElementsTable.SearchBar = (function()
	...
	function Element:New(Idx, Config)
		...
		Config.Debounce    = Config.Debounce or 0
		...
		Creator.AddSignal(SearchBox:GetPropertyChangedSignal("Text"), function()
			SB.Value = SearchBox.Text
			if Config.Debounce > 0 then
				if debounceTask then task.cancel(debounceTask) end
				debounceTask = task.delay(Config.Debounce, function()
					Library:SafeCallback(SB.Callback, SB.Value)
					Library:SafeCallback(SB.Changed, SB.Value)
				end)
			else
				Library:SafeCallback(SB.Callback, SB.Value)
				Library:SafeCallback(SB.Changed, SB.Value)
		...
		Creator.AddSignal(SearchBox.Focused, function()
			TweenService:Create(SearchBox, TweenInfo.new(0.15), { BackgroundTransparency = 0.7 }):Play()
		end)
		Creator.AddSignal(SearchBox.FocusLost, function()
			TweenService:Create(SearchBox, TweenInfo.new(0.15), { BackgroundTransparency = 0.88 }):Play()
		end)

		function SB:SetValue(v)
			SB.Value = v
			SearchBox.Text = v
...
-- [NEW-2] StatusIndicator — dot สี + label แสดงสถานะ
...
-- [NEW-5] NotifBadge — badge มีตัวเลข + label อัปเดตได้
ElementsTable.NotifBadge = (function()
	...
	function Element:New(Idx, Config)
		...
		local NB = { Type = "NotifBadge", Count = Config.Count }
		...
		function NB:SetCount(n)
			self.Count = n
			CountLabel.Text = n > 99 and "99+" or tostring(n)
			-- pulse on update
			TweenService:Create(Pill, TweenInfo.new(0.1), { Size = UDim2.fromOffset(0, 26) }):Play()
			task.delay(0.1, function()
				if Pill and Pill.Parent then
					TweenService:Create(Pill, TweenInfo.new(0.15), { Size = UDim2.fromOffset(0, 22) }):Play()
				end
			end)
		end
		function NB:Increment(by) NB:SetCount(self.Count + (by or 1)) end
		function NB:Reset()       NB:SetCount(0) end
...
-- [NEW-6] SpeedDial — ปุ่มหลักโผล่ปุ่มย่อย
ElementsTable.SpeedDial = (function()
	...
	function Element:New(Idx, Config)
		Config = Config or {}
		assert(Config.Title,   "SpeedDial - Missing Title")
		assert(Config.Actions, "SpeedDial - Missing Actions")
		Config.ButtonText = Config.ButtonText or "Actions"

		local SD = { Type = "SpeedDial", Open = false }
		local SDFrame = Components.Element(Config.Title, Config.Description, self.Container, false, Config)
		SD.SetTitle = SDFrame.SetTitle
		SD.SetDesc  = SDFrame.SetDesc
		SD.Visible  = SDFrame.Visible
		SD.Elements = SDFrame
		...
		-- Sub buttons holder — แสดงใน LabelHolder
		...
		for i, action in ipairs(Config.Actions) do
			local iconImg = action.Icon and Library:GetIcon(action.Icon) or ""
			...
			-- set icon ทีหลังแยกออกมา ป้องกัน nil ใน children table
			if iconImg ~= "" then
			...
			local Motor, SetT = Creator.SpringMotor(0.88, SubBtn, "BackgroundTransparency")
			Creator.AddSignal(SubBtn.MouseEnter,        function() SetT(0.72) end)
			Creator.AddSignal(SubBtn.MouseLeave,        function() SetT(0.88) end)
			Creator.AddSignal(SubBtn.MouseButton1Click, function()
				Library:SafeCallback(action.Callback)
			end)
		end
		...
		Creator.AddSignal(MainBtn.MouseButton1Click, function()
			SD.Open = not SD.Open
			SubHolder.Visible = SD.Open
			MainBtn.Text = Config.ButtonText .. (SD.Open and " ▴" or " ▾")
		end)

		function SD:Destroy() SDFrame.Frame:Destroy() if Idx then Library.Options[Idx] = nil end end
		if Idx then Library.Options[Idx] = SD end
		return SD
	end
	return Element
end)()
...
return Library, SaveManager, InterfaceManager, Mobil
		SearchBar = {
			Save = function(idx, object)
