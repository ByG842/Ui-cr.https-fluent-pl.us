...
local Themes = {
	...
	Darker = {
		...
		AcrylicGradient = ColorSequence.new(Color3.fromRGB(14, 14, 18), Color3.fromRGB(18, 18, 24)),
		AcrylicNoise = 0.94,
		TitleBarLine = Color3.fromRGB(48, 48, 60),
		Tab = Color3.fromRGB(90, 90, 110),
		Element = Color3.fromRGB(55, 55, 70),
		ElementBorder = Color3.fromRGB(20, 20, 28),
		InElementBorder = Color3.fromRGB(45, 45, 58),
		ElementTransparency = 0.82,
		ToggleSlider = Color3.fromRGB(80, 80, 100),
		ToggleToggled = Color3.fromRGB(20, 20, 28),
		SliderRail = Color3.fromRGB(70, 70, 90),
		DropdownFrame = Color3.fromRGB(100, 100, 125),
		DropdownHolder = Color3.fromRGB(28, 28, 36),
		DropdownBorder = Color3.fromRGB(20, 20, 28),
		DropdownOption = Color3.fromRGB(80, 80, 100),
		Keybind = Color3.fromRGB(70, 70, 90),
		Input = Color3.fromRGB(70, 70, 90),
		InputFocused = Color3.fromRGB(8, 8, 12),
		InputIndicator = Color3.fromRGB(90, 90, 115),
		...
		DialogInput = Color3.fromRGB(36, 36, 48),
		DialogInputLine = Color3.fromRGB(90, 90, 115),
		Text = Color3.fromRGB(235, 235, 245),
		SubText = Color3.fromRGB(155, 155, 180),
	...
	AMOLED = {
		...
		AcrylicMain = Color3.fromRGB(4, 4, 6),
		AcrylicBorder = Color3.fromRGB(28, 24, 40),
		AcrylicGradient = ColorSequence.new(Color3.fromRGB(2, 2, 4), Color3.fromRGB(5, 4, 8)),
		AcrylicNoise = 1,
		TitleBarLine = Color3.fromRGB(22, 18, 32),
		Tab = Color3.fromRGB(50, 42, 72),
		Element = Color3.fromRGB(14, 12, 20),
		ElementBorder = Color3.fromRGB(5, 4, 8),
		InElementBorder = Color3.fromRGB(32, 26, 48),
		ElementTransparency = 0.92,
		ToggleSlider = Color3.fromRGB(45, 38, 65),
		ToggleToggled = Color3.fromRGB(180, 160, 255),
		SliderRail = Color3.fromRGB(38, 32, 55),
		DropdownFrame = Color3.fromRGB(28, 24, 40),
		DropdownHolder = Color3.fromRGB(6, 5, 10),
		DropdownBorder = Color3.fromRGB(22, 18, 32),
		DropdownOption = Color3.fromRGB(42, 35, 60),
		Keybind = Color3.fromRGB(38, 32, 55),
		Input = Color3.fromRGB(18, 15, 26),
		InputFocused = Color3.fromRGB(2, 2, 4),
		InputIndicator = Color3.fromRGB(52, 44, 75),
		InputIndicatorFocus = Color3.fromRGB(180, 160, 255),
		Dialog = Color3.fromRGB(6, 5, 10),
		DialogHolder = Color3.fromRGB(4, 4, 8),
		DialogHolderLine = Color3.fromRGB(18, 15, 26),
		DialogButton = Color3.fromRGB(12, 10, 18),
		DialogButtonBorder = Color3.fromRGB(32, 26, 48),
		DialogBorder = Color3.fromRGB(26, 22, 38),
		DialogInput = Color3.fromRGB(14, 12, 20),
		DialogInputLine = Color3.fromRGB(50, 42, 72),
		Text = Color3.fromRGB(225, 215, 255),
		SubText = Color3.fromRGB(145, 125, 200),
...
Components.Notification = (function()
	...
	function Notification:New(Config)
		...
		local CORNER = 12  -- รัศมีความโค้งหลัก

		-- ── พื้นหลังหลัก (โค้งทุกมุม ไม่มีรอยต่อ) ─────────────
		...
		-- ── แถบสีซ้าย (โค้งด้านซ้ายพอดีกับ corner) ──────────────
		...
		NewNotification.CloseButton = New("TextButton", {
			...
			Size             = UDim2.fromOffset(16, 16),
			BackgroundColor3 = Color3.fromRGB(60, 50, 85),
			BackgroundTransparency = 0.3,
			BorderSizePixel  = 0,
			ZIndex           = 5,
		}, {
			New("UICorner", { CornerRadius = UDim.new(1, 0) }),  -- วงกลมสมบูรณ์
		})

		-- ── Progress bar ──────────────────────────────────────────
		local ProgressTrack = New("Frame", {
			Size             = UDim2.new(1, -(CORNER * 2), 0, 2),
			Position         = UDim2.new(0, CORNER, 1, -5),
			BackgroundColor3 = Color3.fromRGB(40, 35, 58),
		...
		-- ── Root (ClipsDescendants = true ตัดส่วนเกินออก) ────────
		NewNotification.Root = New("Frame", {
			BackgroundTransparency = 1,
			Size             = UDim2.new(1, 0, 1, 0),
			Position         = UDim2.fromScale(1, 0),
			ClipsDescendants = true,  -- ป้องกันรอยต่อ/ล้นขอบ
		}, {
		...
		-- ── วาง Layout frame แยก เพื่อจัด position ─────────────
		local ContentFrame = New("Frame", {
			BackgroundTransparency = 1,
			Size             = UDim2.new(1, 0, 1, -10),  -- เว้นล่างให้ progress
			Position         = UDim2.new(0, 0, 0, 0),
			ZIndex           = 2,
			Parent           = NewNotification.Root,
		})

		-- จัด IconCircle กับ Title ให้ยืดหยุ่นตามความสูง
		...
		IconCircle.Parent   = TopBar
		NewNotification.Title.Position = UDim2.new(0, 44, 0.5, 0)
		NewNotification.Title.AnchorPoint = Vector2.new(0, 0.5)
		NewNotification.Title.Parent = TopBar
		NewNotification.CloseButton.Parent = TopBar

		NewNotification.LabelHolder.Position = UDim2.new(0, 10, 0, 36)
		NewNotification.LabelHolder.Size = UDim2.new(1, -20, 0, 0)
		NewNotification.LabelHolder.Parent = ContentFrame

		TypeBar.Parent      = NewNotification.Root
		ProgressTrack.Parent = NewNotification.Root
		...
		function NewNotification:Open()
			...
			local hasContent = Config.Content ~= "" or Config.SubContent ~= ""
			local totalH = hasContent and math.max(58, 42 + contentH + 12) or 46
			NewNotification.Holder.Size = UDim2.new(1, 0, 0, totalH + 8)

			-- ปรับตำแหน่ง IconCircle และ Title ให้กลาง
			if not hasContent then
				TopBar.Size = UDim2.new(1, 0, 1, -10)
			end

			RootMotor:setGoal({
				Scale  = Spring(0, { frequency = 7 }),
			...
			if Config.Duration and Config.Duration > 0 then
				...
				conn = game:GetService("RunService").Heartbeat:Connect(function()
					if NewNotification.Closed then conn:Disconnect() return end
					local pct = 1 - math.clamp((tick() - start) / dur, 0, 1)
					ProgressFill.Size = UDim2.new(pct, 0, 1, 0)
					if pct <= 0 then conn:Disconnect() end
				end)
			else
		...
		function NewNotification:Close()
			if not NewNotification.Closed then
				...
				task.spawn(function()
					RootMotor:setGoal({
						Scale  = Spring(1, { frequency = 7 }),
						Offset = Spring(80, { frequency = 7 }),
					})
					task.wait(0.3)
